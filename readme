...
Prerequisites:
	...
	make:
		The gnu make utility is needed to compile gnu software packages.
	gcc:
		The gnu c compiler is required to compile most gnu software packages.
	perl:
		The install package was written for perl 5.10.
...
Pkg Config:
	...
	To change the pkg-config search path use the export command:
